...
sr_shmsub_notify_new_wrlock(sr_sub_shm_t *sub_shm, const char *shm_name, sr_sub_event_t lock_event, sr_cid_t cid)
{
    ...
    /* WRITE LOCK */
    if ((err_info = sr_rwlock(&sub_shm->lock, SR_MAIN_LOCK_TIMEOUT * 1000, SR_LOCK_WRITE, cid, __func__, NULL, NULL))) {
        return err_info;
    }
    ...
    ret = 0;
    while (!ret && (sub_shm->event && (sub_shm->event != lock_event))) {
        /* COND WAIT */
    ...
    /* we have write lock and the desired (no) event */
    return NULL;
        ...
        sr_cid_t cid, sr_error_info_t **cb_err_info)
{
    ...
    ret = 0;
    while (!ret && (sub_shm->lock.readers[0] || (!SR_IS_NOTIFY_EVENT(sub_shm->event) && sub_shm->event))) {
        /* COND WAIT */
        ret = pthread_cond_timedwait(&sub_shm->lock.cond, &sub_shm->lock.mutex, &timeout_ts);
    }
...
sr_shmsub_change_notify_clear(struct sr_mod_info_s *mod_info)
{
    ...
    while ((mod = sr_modinfo_next_mod(mod, mod_info, mod_info->diff, &aux))) {
        ...
        /* SUB WRITE LOCK */
        if ((err_info = sr_rwlock(&multi_sub_shm->lock, SR_MAIN_LOCK_TIMEOUT * 1000, SR_LOCK_WRITE, cid, __func__, NULL,
                NULL))) {
            goto cleanup;
        }
    ...
    while ((mod = sr_modinfo_next_mod(mod, mod_info, mod_info->diff, &aux))) {
        ...
        do {
            if ((opts & SR_SUBSCR_UNLOCKED) && !ext_shm_buf) {
                ...
                SR_CHECK_MEM_GOTO(!ext_shm_buf, err_info, cleanup_wrunlock);
...
{
    sr_error_info_t *err_info = NULL, *cb_err_info;
    sr_rpc_t *shm_rpc;
    char *input_lyb = NULL;
...
sr_error_info_t *
sr_shmsub_notif_notify(const struct lyd_node *notif, time_t notif_ts, sr_sid_t sid, sr_cid_t cid,
        const sr_mod_notif_sub_t *notif_subs, uint32_t notif_sub_count)
{
    ...
    /* SUB WRITE LOCK */
    if ((err_info = sr_shmsub_notify_new_wrlock((sr_sub_shm_t *)multi_sub_shm, ly_mod->name, 0, cid))) {
        goto cleanup;
    }
        ...
        sr_error_info_t **err_info)
{
    ...
    /* SUB READ/WRITE LOCK */
    if ((*err_info = sr_rwlock(&multi_sub_shm->lock, SR_MAIN_LOCK_TIMEOUT * 1000, mode, tmp_sess->conn->cid, __func__,
            NULL, NULL))) {
        return 1;
    }
    ...
    if ((sub_info->event != multi_sub_shm->event) || (sub_info->request_id != multi_sub_shm->request_id)) {
        ...
        sr_rwunlock(&multi_sub_shm->lock, SR_MAIN_LOCK_TIMEOUT * 1000, mode, tmp_sess->conn->cid, __func__);
...
sr_error_info_t *
sr_shmsub_change_listen_dismiss_event(sr_multi_sub_shm_t *multi_sub_shm, struct modsub_changesub_s *sub, sr_cid_t cid)
{
    sr_error_info_t *err_info = NULL;
    ...
    /* SUB WRITE UNLOCK */
    sr_rwunlock(&multi_sub_shm->lock, SR_MAIN_LOCK_TIMEOUT * 1000, SR_LOCK_WRITE, cid, __func__);
...
sr_shmsub_change_listen_process_module_events(struct modsub_change_s *change_subs, sr_conn_ctx_t *conn)
{
    ...
    /* SUB READ UPGR LOCK */
    if ((err_info = sr_rwlock(&multi_sub_shm->lock, SR_MAIN_LOCK_TIMEOUT * 1000, SR_LOCK_READ_UPGR, conn->cid, __func__,
            NULL, NULL))) {
        goto cleanup;
    }
...
process_event:
        ...
        sr_rwunlock(&multi_sub_shm->lock, SR_MAIN_LOCK_TIMEOUT * 1000, SR_LOCK_READ_UPGR, conn->cid, __func__);
    ...
    if ((err_info = sr_rwrelock(&multi_sub_shm->lock, SR_MAIN_LOCK_TIMEOUT * 1000, SR_LOCK_WRITE, conn->cid, __func__,
            NULL, NULL))) {
        goto cleanup_rdunlock;
    }
        ...
        sr_error_t err_code, sr_error_info_t **err_info)
{
    ...
    /* SUB READ/WRITE LOCK */
    if ((*err_info = sr_rwlock(&sub_shm->lock, SR_MAIN_LOCK_TIMEOUT * 1000, mode, cid, __func__, NULL, NULL))) {
        return 1;
    }
    ...
    if ((SR_SUB_EV_OPER != sub_shm->event) || (exp_req_id != sub_shm->request_id)) {
        /* SUB READ/WRITE UNLOCK */
        sr_rwunlock(&sub_shm->lock, SR_MAIN_LOCK_TIMEOUT * 1000, mode, cid, __func__);
...
sr_error_info_t *
sr_shmsub_oper_listen_dismiss_event(sr_sub_shm_t *sub_shm, struct modsub_opersub_s *sub, sr_cid_t cid)
{
    sr_error_info_t *err_info = NULL;
    uint32_t data_len;
    char *data;
    ...
    /* SUB WRITE UNLOCK */
    sr_rwunlock(&sub_shm->lock, SR_MAIN_LOCK_TIMEOUT * 1000, SR_LOCK_WRITE, cid, __func__);
...
sr_shmsub_oper_listen_process_module_events(struct modsub_oper_s *oper_subs, sr_conn_ctx_t *conn)
{
    ...
    for (i = 0; (err_code == SR_ERR_OK) && (i < oper_subs->sub_count); ++i) {
        ...
        /* SUB READ LOCK */
        if ((err_info = sr_rwlock(&sub_shm->lock, SR_MAIN_LOCK_TIMEOUT * 1000, SR_LOCK_READ, conn->cid, __func__,
                NULL, NULL))) {
            goto error;
        }
        ...
        if ((sub_shm->event != SR_SUB_EV_OPER) || (sub_shm->request_id == oper_sub->request_id)) {
            /* SUB READ UNLOCK */
            sr_rwunlock(&sub_shm->lock, SR_MAIN_LOCK_TIMEOUT * 1000, SR_LOCK_READ, conn->cid, __func__);
            continue;
        }
        request_id = sub_shm->request_id;
...
error_wrunlock:
    /* SUB WRITE UNLOCK */
    sr_rwunlock(&sub_shm->lock, SR_MAIN_LOCK_TIMEOUT * 1000, SR_LOCK_WRITE, conn->cid, __func__);
    goto error;
...
sr_error_info_t *
sr_shmsub_notif_listen_dismiss_event(sr_multi_sub_shm_t *multi_sub_shm, uint32_t request_id, sr_cid_t cid)
{
    sr_error_info_t *err_info = NULL;
...
sr_shmsub_notif_listen_process_module_events(struct modsub_notif_s *notif_subs, sr_conn_ctx_t *conn)
{
    ...
    /* SUB READ LOCK */
    if ((err_info = sr_rwlock(&multi_sub_shm->lock, SR_MAIN_LOCK_TIMEOUT * 1000, SR_LOCK_READ, conn->cid, __func__,
            NULL, NULL))) {
        goto cleanup;
    }
    ...
    /* SUB READ UNLOCK */
    sr_rwunlock(&multi_sub_shm->lock, SR_MAIN_LOCK_TIMEOUT * 1000, SR_LOCK_READ, conn->cid, __func__);

    SR_LOG_INF("Processing \"notif\" \"%s\" event with ID %u.", notif_subs->module_name, multi_sub_shm->request_id);

    /* SUB WRITE LOCK */
    if ((err_info = sr_rwlock(&multi_sub_shm->lock, SR_MAIN_LOCK_TIMEOUT * 1000, SR_LOCK_WRITE, conn->cid, __func__,
            NULL, NULL))) {
        goto cleanup;
    }
