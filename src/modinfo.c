 ...
 */
static sr_error_info_t *
sr_module_oper_data_update(struct sr_mod_info_mod_s *mod, sr_sid_t *sid, sr_conn_ctx_t *conn, const char *request_xpath,
...
#define PATH_LEN 128
    ...
    for (i = 0; i < SR_DS_COUNT; ++i) {
        snprintf(path, PATH_LEN, "lock[cid='%%"PRIu32"'][datastore='ietf-datastores:%s']/lock-mode", sr_ds2str(i));
        if ((err_info = sr_modinfo_module_srmon_locks(&shm_mod->data_lock_info[i].lock, path, sr_mod))) {
            return err_info;
        }
    }
#undef PATH_LEN
...
sr_modinfo_module_srmon_connections(sr_main_shm_t *main_shm, struct lyd_node *sr_state)
{
    ...
    if ((err_info = sr_conn_info(&cids, &pids, &conn_count))) {
        return err_info;
    }
        ...
        const char *request_xpath, uint32_t timeout_ms, sr_get_oper_options_t opts, sr_error_info_t **cb_error_info)
{
    ...
    if (!mod_info->data_cached) {
        ...
        if (mod_info->ds == SR_DS_OPERATIONAL) {
            ...
            if ((err_info = sr_module_oper_data_update(mod, sid, conn, request_xpath, timeout_ms, opts, &mod_info->data,
                    cb_error_info))) {
                return err_info;
            }
